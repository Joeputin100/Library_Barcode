...
def clean_call_number(
    call_num_str, genres, google_genres=None, title="", is_original_data=False
):
    ext_logger.debug(
    ...
    if not isinstance(call_num_str, str):
        ext_logger.debug(
            f"clean_call_number returning empty string for non-string input: {call_num_str}"
        )
    ...
    if (
        any(g.lower() in fiction_keywords_all for g in google_genres)
        or any(genre.lower() in fiction_keywords_all for genre in genres)
        or any(keyword in title.lower() for keyword in fiction_keywords_all)
    ):
        ext_logger.debug(
            f"clean_call_number returning FIC based on genre/title keywords: {cleaned}"
        )
        return "FIC"
    ...
    if cleaned.lower() in [
        ...
        ext_logger.debug(
            f"clean_call_number returning FIC based on cleaned string match: {cleaned}"
        )
        return "FIC"
    ...
    if cleaned.upper().startswith("FIC"):
        ext_logger.debug(
            f"clean_call_number returning FIC based on explicit FIC: {cleaned}"
        )
        return "FIC"
    ...
    if re.match(r"^[A-Z]{1,3}\d+(\.\d+)?$", cleaned) or re.match(
        r"^\d+(\.\d+)?$", cleaned
    ):
    ...
    ext_logger.debug(
        f"clean_call_number returning empty string for invalid format: {cleaned}"
    )
    return ""
...
def get_book_metadata_google_books(title, author, cache):
    ...
    try:
        ...
        data = response.json()
        ext_logger.debug(
            f"Google Books raw response for '{title}' by '{author}': {response.text}"
        )
        ...
        if "items" in data and data["items"]:
            ...
            ext_logger.info(
                f"Google Books genres for '{title}': {volume_info.get('categories', [])}"
            )
        ...
        for i in range(len(retry_delays) + 1):
            try:
                ...
                if i < len(retry_delays):
                    ext_logger.warning(
                        f"Vertex AI batch call failed: {e}. Retrying in {retry_delays[i]} seconds..."
                    )
                    time.sleep(retry_delays[i])
                else:
                    ext_logger.error(
                        f"Vertex AI batch call failed after multiple retries: {e}"
                    )
