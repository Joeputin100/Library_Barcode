...
def test_enrich_book_data_isbn(
    mock_load_cache, mock_save_cache, mock_get_book_metadata
):
...
@patch("book_importer.save_cache")
@patch("book_importer.load_cache")
def test_enrich_book_data_title_author(
    mock_load_cache, mock_save_cache, mock_get_book_metadata
):
...
def test_insert_books_to_bigquery_success(mock_bigquery_client):
    ...
    # Call the function
    insert_books_to_bigquery(books, mock_instance)

    # Assertions
    mock_instance.insert_rows_json.assert_called_once_with(
        "barcode.new_books", books
    )
