...
def convert_df_to_marc(df):
    ...
    for index, row in df.iterrows():
        ...
        record.add_field(
            Field(tag="008", data="230101s2023    xxu           000 0 eng d")
        )

        # ISBN with Price
        if "isbn" in row and row["isbn"]:
            subfields = ["a", row["isbn"]]
            if "price" in row and row["price"]:
                subfields.extend(["c", f"${row['price']}"])
            record.add_field(
                Field(
                    tag="020",
                    indicators=[" ", " "],
        ...
        if "publication_date" in row and row["publication_date"]:
            record.add_field(
                Field(
                    tag="264",
                    indicators=[" ", "1"],
                    subfields=["c", row["publication_date"]],
                )
            )
        ...
        record.add_field(
            Field(
                tag="300",
                indicators=[" ", " "],
                subfields=["a", "1 volume (various pagings)"],
            )
        )
        record.add_field(
            Field(
        ...
        # Holding Information - Using LOCAL call number for your system
        if "holding_barcode" in row and row["holding_barcode"]:
            holding_field = Field(
                ...
                subfields=[
                    ...
                    "s",
                    "h",
                    row.get("local_call_number", row.get("call_number", "")),  # Prefer local call number
                    "p",
                    row["holding_barcode"],
                ],
            )
        ...
        # Add LOC call number as reference in 090 field
        if "call_number" in row and row["call_number"] and row["call_number"] != row.get("local_call_number", ""):
            record.add_field(
                Field(
                    tag="090",
                    indicators=[" ", " "],
                    subfields=["a", row["call_number"]],
                )
            )
