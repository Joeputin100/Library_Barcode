...
def parse_query_with_duet(query_string: str) -> str:
    """
    Uses the Duet AI API to parse a natural language query into a structured JSON format.
    """
    credentials, project_id = google.auth.load_credentials_from_file("vertex ai key static-webbing-461904-c4-aa8f7ec18c62.json")
    ...
    example_range_b000100_b000200 = '{ "type": "barcode_range", "start": "B000100", "end": "B000200" }'
    example_starts_with_b = '{ "type": "barcode_starts_with", "prefix": "B" }'
    example_author = '{ "type": "field_query", "field": "author", "value": "brandon sanderson" }'
    example_series = '{ "type": "field_query", "field": "series", "value": "stormlight archive" }'
...
For `barcode_range` and `barcode` queries, extract the numeric part of the barcode for `start`, `end` and `value` values. For example, \"b1\" should be \"1\".
